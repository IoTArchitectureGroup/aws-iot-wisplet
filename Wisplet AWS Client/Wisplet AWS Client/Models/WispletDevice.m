...
-(void)subscribeToMqttTopics
{
    ...
    [iotDataManager subscribeToTopic:topicAlertMessage qos:0 messageCallback:^(NSData *payload) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            // Parse out PID index, and Rule text, for display on-screen in a popup (toast) alert.
            
            // Example payload: { "PID": 4, "PIDName": "p4", "PIDValue": "3.30", "T1": "3.00", "T2": "0.00", "Rule": "If Greater Than or Equal to T1", "RuleNumber": 7 }
            // (Note, for a fresh Wisplet kit, instead of PIDName values being "p0", "p1", ... "p4", they will be
            // "TemperatureDegF", "Humidity", "Lite", "IR", "Pot", "STAT")
...
-(void)populateFromDictionary:(NSDictionary*)chargerDictionary {
    ...
    // Parse using sensor names that will be used if Wisplet has had a new
    // ruleset pushed to it via the IoT Architect tool portion of the
    // server at http://wisplet.net
    // Sensor names are: p0, p1, p2, p3, p4, p5.
    ...
    // End using sensor names that will be used if Wisplet HAS had a new
    // ruleset pushed to it.
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    
    // Look again, for sensor names that are generated by a FRESH Wisplet
    // that has never had a ruleset pushed to it:
    // Sensor names are: "TemperatureDegF", "Humidity", "Lite", "IR", "Pot", "STAT".
    
    if(chargerDictionary[@"TemperatureDegF"]) // temp
    {
        NSString *value = [chargerDictionary valueForKey:@"TemperatureDegF"];
        [self.boardModel.temperatureSensor setCurrentValueString:value];
        float floatValue = [value floatValue];
        [self.boardModel.temperatureSensor setCurrentValue:floatValue];
    }
    
    if(chargerDictionary[@"Humidity"]) // humidity
    {
        NSString *value = [chargerDictionary valueForKey:@"Humidity"];
        [self.boardModel.humiditySensor setCurrentValueString:value];
        float floatValue = [value floatValue];
        [self.boardModel.humiditySensor setCurrentValue:floatValue];
    }
    
    if(chargerDictionary[@"Lite"]) // visible light
    {
        NSString *value = [chargerDictionary valueForKey:@"Lite"];
        [self.boardModel.visibleLightSensor setCurrentValueString:value];
        float floatValue = [value floatValue];
        [self.boardModel.visibleLightSensor setCurrentValue:floatValue];
    }
    
    if(chargerDictionary[@"IR"]) // ir light
    {
        NSString *value = [chargerDictionary valueForKey:@"IR"];
        [self.boardModel.irLightSensorSensor setCurrentValueString:value];
        float floatValue = [value floatValue];
        [self.boardModel.irLightSensorSensor setCurrentValue:floatValue];
    }
    
    if(chargerDictionary[@"Pot"]) // potentiometer
    {
        NSString *value = [chargerDictionary valueForKey:@"Pot"];
        [self.boardModel.potentiometerSensor setCurrentValueString:value];
        float floatValue = [value floatValue];
        [self.boardModel.potentiometerSensor setCurrentValue:floatValue];
    }
    
    if(chargerDictionary[@"STAT"]) // stat
    {
        NSString *value = [chargerDictionary valueForKey:@"STAT"];
        [self.boardModel.statDisplaySensor setCurrentValueString:value];
        int intValue = [value intValue];
        [self.boardModel.statDisplaySensor setCurrentValue:intValue];
    }
    
    // End using sensor names that will be used if Wisplet has never
    // had a ruleset pushed to it
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
